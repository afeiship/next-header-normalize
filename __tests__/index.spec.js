...
describe('api.basic test', () => {
  ...
  test('two same content-type', function () {
    const obj = {
      'content-type': 'application/json; charset=utf-8',
      'Content-Type': 'www-form-urlencoded'
    };

    const result1 = nx.headerNormalize(obj, { lowerCase: true });
    const result2 = nx.headerNormalize(obj);
    expect(result1).toEqual({ 'content-type': 'www-form-urlencoded' });
    expect(result2).toEqual({ 'Content-Type': 'www-form-urlencoded' });
  });
});
